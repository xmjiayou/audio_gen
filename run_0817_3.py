...
def pick_best(ref_wav: Path, text: str, cand_wavs: list, args, scorer=None,
    ...
    wers=[]; sims=[]; cms=[]
    for w in cand_wavs:
        try: werr,_ = asr_wer(w, text, args); wers.append(werr); print(f"{w}::scores:\n"); print("werr: ", werr); print(" ||")
        except Exception: wers.append(0.5)
        #try: sims.append(spk_similarity(ref_wav, w))
        try: sims_tmp = spk_similarity(ref_wav, w); sims.append(sims_tmp); print("sim:", sims_tmp); print(" ||")
        except Exception: sims.append(0.0)
        if scorer is not None:
            #try: cms.append(scorer.score(w))
            try: cms_tmp = scorer.score(w); cms.append(cms_tmp); print("aasist:", cms); print(" ||\n")
...
def main():
    ...
    scorer = None
    if args.aasist_repo and args.aasist_ckpt:
        if args.python_aasist:  # 推荐：用外部环境
            try:
                scorer = ExternalAASISTScorer(args.python_aasist, args.aasist_cli, 
