...
########################################################################
    def _echo_row(self, row: dict):
        def fmt(x, nd=3):
            if x is None: return "NA"
            if isinstance(x, (int, float)): 
                return f"{x:.{nd}f}"
            return str(x)
        # 你也可以用 tqdm.write 避免干扰进度条；这里直接 print
        line = "[score] " + \
              " | ".join([
              f"utt={fmt(row.get('utt'),0)}",
              f"model={fmt(row.get('model'))}",
              f"WER={fmt(row.get('wer'))}",
              f"SIM={fmt(row.get('sim'))}",
              #f"NAT={fmt(row.get('nat'))}",
              f"AASIST={fmt(row.get('aasist'))}",
              #f"Dur={fmt(row.get('duration_s'))}s",
              f"TOTAL={fmt(row.get('score'))}",
            ])
        try:
            from tqdm import tqdm
            tqdm.write(line)   # line 是拼好的那段字符串
        except Exception:
    ...
    # # -------- 评分主流程里，在每个候选完成后调用 _echo_row --------
    # def score_candidates(self, utt: str, candidates: list, ref_embed: str, asr_ref: str):
    #     """
    #     candidates: list[{"name": "idx/f5/xtts/piper", "wav": "/path/to.wav"}]
    #     """
    #     # ... 你原有的循环/计算 wer/sim/nat/aasist 的代码
    #     best = None
    #     best_score = -1e9
    ...
    #         # 你原有的单项评分（注意 None 安全）
    #         wer = self._score_wer(wav_path, asr_ref)          # 越低越好
    #         sim = self._score_similarity(wav_path, ref_embed) # 越高越好
    #         nat = self._score_naturalness(wav_path)           # 越高越好（如果没有就返回 None）

    #         aas = None
    #         if self.use_aasist:
    #             aas = self._score_aasist(wav_path)            # 越低越好（欺诈概率 or logit）

    #         dur = self._wav_duration_s(wav_path)

    #         # 归一化与加权（保持你原来的策略）
    #         total = self._combine(wer=wer, sim=sim, nat=nat, aasist=aas)

    #         row = {
    #             "utt": utt, "model": name, "wav_path": wav_path,
    #             "wer": wer, "sim": sim, "nat": nat, "aasist": aas,
    #             "duration_s": dur, "score": total
    #         }
    #         self._append_log(row)
    #         if self.echo:    # ★★ 每个候选一完成就打印
    #             self._echo_row(row)
    ...
    def score_candidates(
        ...
        for i, (wav_path, tag) in enumerate(candidates):
            ...
            scores.append(final)
            row = {
                "utt": utt, "model": tag, "wav_path": wav_path,
                "wer": wer_norm, "sim": sim_norm, "aasist": aas_norm,
                "score": final
            }
            if self.echo:    # ★★ 每个候选一完成就打印
                self._echo_row(row)
