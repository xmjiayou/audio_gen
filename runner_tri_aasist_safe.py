...
# -*- coding: utf-8 -*-
"""
ALWAYS-200 SAFE RUNNER (no XTTS / no Coqui):
- Backends: IndexTTS (main) + F5-TTS (fallback)
- Extra safety: ref-mimic collage synthesizer (pure offline) -> last-resort tone
- Sentence-wise synth + 10ms crossfade & fade-in/out
- Scoring: faster-whisper (default; can disable) + ECAPA cosine + (optional) AASIST
- Final sweep ensures exactly N outputs (default 200)
"""

import os, sys, shlex, shutil, subprocess, zipfile, warnings, gc, tempfile, math, random
...
# base deps (no TTS pkg beyond f5-tts / indextts)
...
# ---------- duration estimate for fallback ----------
def estimate_duration(text: str):
    t = str(text)
    # 汉字/字符 0.18s，空白/标点加额外停顿
    base = 0.18 * sum(1 for c in t if c.strip())
    bonus = 0.2 * sum(1 for c in t if c in "。！？!?")
    dur = max(0.6, min(15.0, base + bonus))
    return dur

# ---------- ASR: faster-whisper preferred ----------
_faster_model = None
...
def ref_mimic_collage(ref_wav: Path, target_seconds: float, out_wav: Path, sr=16000):
    """Builds a pseudo-speech collage from ref: cut small voiced chunks, random order, crossfade."""
    y, sro = sf.read(ref_wav)
    if y.ndim>1: y=y.mean(axis=1)
    if sro != sr: y = librosa.resample(y, sro, sr)
    y = y.astype(np.float32)
    if len(y) < int(0.2*sr):  # too short
        return write_tone_placeholder(out_wav, seconds=max(0.6, target_seconds), sr=sr)

    # crude voiced detection by energy threshold
    ...
    rng = random.Random(len(y))  # deterministic-ish
    chunks=[]
    total=0
    xfade = int(0.01*sr)
    ...
    # light EQ: band-limit 70–7000 Hz to be more speech-like
    try:
        import scipy.signal as ss  # optional; if missing, skip
...
def main():
    ...
    try:
        ...
        print("IndexTTS init failed, will rely on F5-TTS + safety:", e)
    f5  = F5TTSCLIBackend()
    ...
    errors=[]; used_collage=0; used_tone=0
    for _, row in tqdm(df.iterrows(), total=len(df), desc="Synthesize"):
        ...
        if not cands:
            # OFFLINE safety: ref collage, then tone
            try:
    ...
    # final sweep: ensure all wavs exist & are non-trivial
    ...
    print(f"Safety collage used: {used_collage}, tone placeholders: {used_tone}, final sweep filled: {fill_ct}")
    if errors:
